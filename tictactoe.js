...
function computerTurn() {
    let buttonsClone = [].slice.call(buttons);
    let tries = 0;
    for (i = 0; i < 9; i++) {
        let ran = Math.floor(Math.random() * buttonsClone.length)
        let selection = buttonsClone[ran];
...
function checkWinner(team) {
    for (let i = 0; i < winningRows.length; i++) {
        if (winningRows[i][0].innerHTML === team && winningRows[i][1].innerHTML === team && winningRows[i][2].innerHTML === team) {
            if (team === 'X') {
                endGame('X');
            } else {
                endGame('O');
